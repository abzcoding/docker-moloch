FROM alpine:edge
RUN apk add --update wget curl openjdk8 python git build-base flex bison libpcrecpp lua-uuid linux-headers libmagic libffi-dev zlib-dev gettext json-glib-dev geoip-dev && \
    mkdir -p /data/git &&\
    echo "git clone" &&\
